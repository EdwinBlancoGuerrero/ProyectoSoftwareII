...
class InicioM extends ConexionBD{
	...
	static public function MostrarInicioM($tablaBD, $id){
		...
		$pdo = ConexionBD::conectarBD()->prepare("SELECT id, intro, horaE, horaS, direccion, telefono, correo, logo, favicon FROM $tablaBD WHERE id = :id");
	...
	static public function ActualizarInicioM($tablaBD, $datosC){
		...
		$pdo = ConexionBD::conectarBD()->prepare("UPDATE $tablaBD SET intro = :intro, direccion = :direccion, horaE = :horaE, horaS = :horaS, telefono = :telefono, correo = :correo, logo = :logo, favicon = :favicon WHERE id = :id");
